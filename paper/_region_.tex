...
(Fig.~\suppstats A), and includes data from a list-learning memory experiment
for up to one hour varying by patient with additional buffer time in
which the patients performed typical bed-ridden hospital
...
and friends, etc.).  For the purposes of the Dataset 1
...
multi-session recordings from 4436 electrodes implanted in the brains
of 40 epilepsy patients~\citep{EzzyEtal17, HoraEtal17, KragEtal17,
  KuceEtal17, LinEtal17, SoloEtal18, WeidEtal18, EzzyEtal18,
  KuceEtal18}.  Each recording session lasted from {0.4}--6.6 hours
...
closely ($r = 0.869, p < 10^{-10}$).  Figure~\ref{fig:recon}B displays a
...
electrode (correlation: $r = 0.909, p < 10^{-10}$).  This example confirms
...
to 0: $t(66) = 25.08, p < 10^{-30}$), indicating that the SuperEEG algorithm
...
per-electrode correlation coefficients to 0: $t(66) = 15.16, p < 10^{-20}$).
...
scores across the two datasets: $r = 0.23, p < 10^{-10}$).  While the
...
about full-brain activity patterns.  Further, regions like the right post
central gyrus, right supramarginal gyrus,
...
\message{ !name(main.tex) !offset(-849) }
