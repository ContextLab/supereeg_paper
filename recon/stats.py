...
def time_by_file_index_chunked(fname, ave_data, known_inds, unknown_inds, electrode_ind, k_flat_removed, electrode, time_series, field='Y'):
    ...
    for i in file_inds:
        ...
        for each in chunker(next_inds, 25000):
            ### this code should incorporate the average voltage of the known (subject) electrodes and the average for the unknown (the other subjects)
            next_predicted = np.squeeze(np.dot(np.dot(Kba, Kaa_inv),
                                               zscore(np.float32(data[field][list(filter(None.__ne__, each)), :])
                                                      [:,k_flat_removed]).T).T[:,electrode_ind[0]])
            next_actual = np.squeeze(zscore(np.float32(data[field][:, [int(electrode)]])[list(filter(None.__ne__, each)), :]))
            next_compare_time = pd.DataFrame({'timepoint': list(filter(None.__ne__, each)), 'actual': next_actual, 'predicted': next_predicted, 'session': i})
...
def time_by_file_index_chunked_local(fname, ave_data, known_inds, unknown_inds, electrode_ind, k_flat_removed, electrode, time_series, field='Y'):
    """
    Session dependent function that calculates that finds either the timeseries or the correlation of the predicted and actual timeseries for a given location chunked by 25000 timepoints
    ...
    for i in file_inds:
        ...
        for each in chunker(next_inds, 25000):
            ### this code should incorporate the average voltage of the known (subject) electrodes and the average for the unknown (the other subjects)
            next_predicted = np.squeeze(np.dot(np.dot(Kba, Kaa_inv),
                                               zscore(np.float32(data[field][filter(lambda v: v is not None, each), :])
                                                      [:,k_flat_removed]).T).T[:,electrode_ind[0]])
            next_actual = np.squeeze(zscore(np.float32(data[field][:, [int(electrode)]])[filter(lambda v: v is not None, each), :]))
            next_compare_time = pd.DataFrame({'timepoint': filter(lambda v: v is not None, each), 'actual': next_actual, 'predicted': next_predicted, 'session': i})
            if block_results.empty:
                block_results = next_compare_time
            else:
                block_results = block_results.append(next_compare_time)
...
def time_by_file_index_bo(bo, ave_data, known_inds, unknown_inds):
    ...
    ----------
    results : numpy array
        Timeseries reconstructed for unknown locations
    ...
    for i in file_inds:
        ...
        for each in chunker(next_inds, 25000):
            ### this code should incorporate the average voltage of the known (subject) electrodes and the average for the unknown (the other subjects)
            next_predicted = np.squeeze(np.dot(np.dot(Kba, Kaa_inv),
                                               zscore(np.float32(bo.get_data().as_matrix()[list(filter(None.__ne__, each)), :])).T).T)
            next_actual = np.squeeze(zscore(np.float32(bo.get_data().as_matrix()[list(filter(None.__ne__, each)), :])))
            if block_results==[]:
                block_results = next_predicted
            else:
                block_results = np.vstack((block_results, next_predicted))
        if results==[]:
            results = block_results
        else:
...
def time_by_file_index_bo_local(bo, ave_data, known_inds, unknown_inds):
    """
    Session dependent function that calculates that finds either the timeseries or the correlation of the predicted and actual timeseries for a given location chunked by 25000 timepoints
    ...
    for i in file_inds:
        ...
        for each in chunker(next_inds, 25000):
            ### this code should incorporate the average voltage of the known (subject) electrodes and the average for the unknown (the other subjects)
            next_predicted = np.squeeze(np.dot(np.dot(Kba, Kaa_inv),
                                               zscore(np.float32(bo.get_data().as_matrix()[filter(lambda v: v is not None, each), :])).T).T)
            next_actual = np.squeeze(zscore(np.float32(bo.get_data().as_matrix()[filter(lambda v: v is not None, each), :])))
            if block_results==[]:
                block_results = next_predicted
            else:
                block_results = np.vstack((block_results, next_predicted))
