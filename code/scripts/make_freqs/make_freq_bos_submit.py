...
filterfreqs = np.array([[2,4], [4,8], [8,12], [12,30], [30, 60], [60,125]])
freqnames = ['delta', 'theta', 'alpha', 'beta', 'lgamma', 'hgamma']


# each job command should be formatted as a string
job_script = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'make_freq_bos.py')
...
def create_job(name, job_command):
    ...
    job_fname = os.path.join(config['scriptdir'], name + '.sh')
