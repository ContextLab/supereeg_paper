...
job_script = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'make_robust_bos.py')

files = glob.glob(os.path.join(config['resultsdir'], '*recon.bo'))

job_commands = list(map(lambda x: x[0]+" "+str(x[1]), zip([job_script]*len(files), files)))

# job_names should specify the file name of each script (as a list, of the same length as job_commands)
job_names = list(map(lambda x: os.path.basename(os.path.splitext(x)[0])+'_robust.sh', files))
